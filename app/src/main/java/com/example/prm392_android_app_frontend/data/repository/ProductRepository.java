...
public class ProductRepository {
    ...
    // --- Search có name (và có thể có filter) ---
    public void search(String name, @Nullable ProductFilter filter, Callback<List<ProductDto>> cb) {
        ...
        addFilterParams(q, filter);

        Log.d("SearchRepo", "🔍 Sending search params: " + q);
        productApi.getProducts(q).enqueue(cb);
    }

    // --- Filter chỉ dựa vào filter, không có name ---
    public void filterProducts(@Nullable ProductFilter filter, Callback<List<ProductDto>> cb) {
        Map<String, String> q = new HashMap<>();
        addFilterParams(q, filter);

        Log.d("SearchRepo", "🔍 Sending filter-only params: " + q);
        productApi.getProducts(q).enqueue(cb);
    }

    // --- Hàm helper chung cho search & filter ---
    private void addFilterParams(Map<String, String> q, @Nullable ProductFilter filter) {
        if (filter == null) return;

        Log.d("SearchRepo", "🔍 Applying filter: " + filter);
